    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded border border-[#dadce0] bg-white p-4 shadow-sm">
          ...
          <select value={plan} onChange={(event) => updateFilter(setPlan, event.target.value)} className="mt-2 w-full rounded border border-[#dadce0] bg-[#f8f9fa] px-4 py-3 text-sm font-medium outline-none">
        ...
        <div className="rounded border border-[#dadce0] bg-white p-4 shadow-sm">
          ...
          <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
            <select value={role} onChange={(event) => updateFilter(setRole, event.target.value)} className="rounded border border-[#dadce0] bg-[#f8f9fa] px-3 py-3 text-sm font-medium outline-none">
              <option value="all">Mọi quyền</option>
              <option value="admin">Admin</option>
              <option value="user">User</option>
            </select>
            <select value={sort} onChange={(event) => updateFilter(setSort, event.target.value)} className="rounded border border-[#dadce0] bg-[#f8f9fa] px-3 py-3 text-sm font-medium outline-none">
      ...
      {error && (
        <div className="rounded border border-rose-200 bg-rose-50 p-4 text-sm font-medium text-rose-700">{error}</div>
      )}
              ...
              {data?.users.length === 0 && !loading ? (
                ...
                <tr key={user.id} className="odd:bg-white even:bg-slate-50/70 hover:bg-indigo-50/80 transition-colors group">
                  ...
                  <td className="text-right">
                    <Link href={`/admin/users/${user.id}`} className="inline-flex items-center gap-2 rounded border border-[#dadce0] bg-white px-3 py-2 text-sm font-medium text-[#1a73e8] hover:bg-[#f8f9fa] transition-colors">
                      Xem chi tiết
                      <ArrowRight size={16} />
                    </Link>
                  </td>
