...
private struct PomodoroStoredState: Codable {
    var runtime: PomodoroRuntimeState?
    var tasks: [FocusTask]
    var selectedTaskId: UUID?
}

package final class PomodoroStateRepository {
    private let fileURL: URL
    private let fileManager: FileManager

    package init(fileURL: URL = PomodoroStoragePaths.stateFile, fileManager: FileManager = .default) {
        self.fileURL = fileURL
        self.fileManager = fileManager
    }

    fileprivate func load() -> PomodoroStoredState {
        guard let data = try? Data(contentsOf: fileURL),
              let state = try? JSONDecoder().decode(PomodoroStoredState.self, from: data) else {
            return PomodoroStoredState(runtime: nil, tasks: [], selectedTaskId: nil)
        }
        return state
    }

    fileprivate func save(_ state: PomodoroStoredState) {
...
private enum PomodoroStoragePaths {
...
package final class PomodoroViewModel: ObservableObject {
    ...
    private let stateRepository: PomodoroStateRepository
    ...
    private func handlePhaseCompletionIfNeeded(
        ...
        scheduledPhaseTaskID = UUID()
        if phase == .focus {
            recordCurrentFocusProgressIfNeeded(referenceDate: activePhaseEndDate)
            learningStatsRecorder.recordCompletedFocusSession(at: activePhaseEndDate, source: .pomodoro)
        }
        remainingSeconds = 0
        phaseEndDate = nil
    ...
    private func recordCurrentFocusProgressIfNeeded(referenceDate: Date) {
        ...
        let intervalEnd = min(referenceDate, phaseEndDate ?? referenceDate)
        let interval = DateInterval(
            start: intervalEnd.addingTimeInterval(-TimeInterval(delta)),
            end: intervalEnd
        )
        learningStatsRecorder.recordFocusedInterval(interval, source: .pomodoro)
    ...
    private func persistRuntimeStateImmediately() {
        ...
        var state = stateRepository.load()
    ...
    private func restoreRuntimeStateIfAvailable(now: Date? = nil) {
        ...
        guard let snapshot = stateRepository.load().runtime,
    ...
    private func loadTasks() {
        ...
        let loadedTasks = state.tasks
        let selection = state.selectedTaskId
