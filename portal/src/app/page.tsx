...
export default function Home() {
  ...
  const primaryHref = isAuthenticated ? '/pro' : '/api/auth/google';
  const freeFeatures = capabilities.filter(c => !c.isProOnly);
  const proFeatures = capabilities.filter(c => c.isProOnly);

  return (
    <main className="landing-page" style={{ background: 'var(--background)', color: 'var(--foreground)' }}>
      {/* Navigation */}
        ...
        <div className="glass" style={{ 
          ...
          <div className="nav-links" style={{ display: 'flex', gap: '32px', alignItems: 'center' }}>
            ...
            <Link href={primaryHref} className="portal-button" style={{ 
              ...
              {isAuthenticated ? 'Vào Portal' : 'Đăng nhập'}
            </Link>
          </div>
        </div>
      </nav>
              ...
              <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
                ...
                <div style={{ marginTop: 'auto', paddingTop: '32px' }}>
                  <div style={{ height: '120px', background: 'var(--surface-soft)', borderRadius: '16px', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                     <p style={{color: 'var(--muted)', fontSize: '0.9rem'}}>Drag & Drop Area</p>
                  </div>
                </div>
              </div>
            </article>
          ...
          <div className="portal-pricing-grid" style={{ 
            ...
            <div className="portal-pricing-card animate-slide-up" style={{ 
              ...
              <ul style={{ listStyle: 'none', display: 'grid', gap: '16px', marginBottom: '40px', flex: 1 }}>
                ...
                {proFeatures.slice(0, 2).map(f => (
                  <li key={f.key} style={{ display: 'flex', gap: '12px', alignItems: 'center', color: 'var(--muted)', opacity: 0.3 }}>
                    <ChevronRight size={18} /> {f.name}
                  </li>
                ))}
              </ul>
            ...
            <div className="portal-pricing-card featured animate-slide-up delay-100" style={{ 
              ...
              <Link href={primaryHref} className="portal-button" style={{ 
                ...
                {isAuthenticated ? 'Nâng cấp ngay' : 'Đăng nhập để nâng cấp'}
              </Link>
            </div>
          </div>
        </section>
