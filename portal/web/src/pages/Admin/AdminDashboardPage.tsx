...
import { Activity, AlertTriangle, BarChart3, ChevronLeft, ChevronRight, CreditCard, Loader2, RefreshCw, ShieldCheck, TrendingUp, Users, type LucideIcon } from 'lucide-react'
...
function StatCard({ title, value, change, isPositive, icon: Icon, color }: { title: string; value: string | number; change?: string; isPositive?: boolean; icon: LucideIcon; color: string }) {
  return (
...
export function AdminDashboardPage() {
  ...
  const pagedRecentEvents = useMemo(() => {
    if (!stats) return []
    const start = (safeRecentEventsPage - 1) * recentEventsPageSize
    return stats.recentEvents.slice(start, start + recentEventsPageSize)
  }, [safeRecentEventsPage, stats])
  ...
  if (error && !stats) {
    return (
      <div className="rounded bg-white border border-[#dadce0] p-8 text-center shadow-sm">
        ...
        <button type="button" onClick={() => void loadStats()} className="mt-4 px-4 py-2 rounded bg-[#1a73e8] text-white text-sm font-medium hover:bg-[#1557b0] transition-colors border-none cursor-pointer">Thử lại</button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 bg-[#f8f9fa] font-sans text-[#202124]">
      ...
      <div className="flex items-center justify-between gap-4 border-b border-[#dadce0] pb-4">
        ...
        <button
          className="inline-flex items-center gap-2 px-4 py-2 rounded text-sm font-medium text-[#1a73e8] border border-[#dadce0] bg-white hover:bg-[#f8f9fa] transition-colors disabled:opacity-60 cursor-pointer"
      ...
      <div className="p-4 rounded bg-white border border-[#dadce0] shadow-sm">
        ...
        {stats.recentEvents.length > recentEventsPageSize && (
          <div className="mt-4 flex flex-col gap-3 border-t border-[#dadce0] pt-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-[#5f6368]">Trang {safeRecentEventsPage} / {recentEventsTotalPages} · {stats.recentEvents.length} event gần đây</p>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setRecentEventsPage((current) => Math.max(current - 1, 1))}
                disabled={safeRecentEventsPage <= 1 || loading}
              ...
              <button
                ...
                disabled={safeRecentEventsPage >= recentEventsTotalPages || loading}
                className="inline-flex items-center gap-2 rounded border border-[#dadce0] bg-white px-3 py-2 text-sm font-medium text-[#3c4043] hover:bg-[#f8f9fa] disabled:opacity-40 cursor-pointer"
              >
                Sau
