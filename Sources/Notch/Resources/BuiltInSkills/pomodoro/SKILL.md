...
Rules:
- Use `notchctl` for Pomodoro control inside Notch. The app intercepts this command internally.
...
- `status` returns the current Pomodoro state as JSON, including remaining time, current phase, and active status. Use this to query the timer state.

Command cookbook:

Check Pomodoro status:
```sh
notchctl focus status
```
...
Pause Pomodoro:
```sh
notchctl focus pause pomodoro
```

Resume Pomodoro:
```sh
notchctl focus resume pomodoro
```

Reset Pomodoro:
```sh
notchctl focus reset pomodoro
```
